...
> [!WARNING]
> ### ⚠️ Advanced Platform Behaviour, V8 Runtime Stability & Workflow Validation
> - **Under the Hood (Google V8 Engine):** Hornbill executes JavaScript expressions using an embedded **Google Chrome V8 JIT** runtime inside its main application processes. While performance is extremely fast, V8 JIT execution in shared processes means rogue or long-running scripts (e.g. infinite loops or uncontrolled recursion) cannot be gracefully interrupted without killing or destabilising core application processes.
> - **Strict Guardrail Guideline (Scalar Expressions Only):** Expressions must be kept strictly to **instantaneous, stateless, single-line scalar transformations** (arithmetic, string formatting, URL encoding, dates, and delimiter slicing). **NEVER write loops (`for`, `while`, `do`), recursive functions, network calls, or memory-heavy scripts within `&[...]`.**
> - **Platform Defensive Risk:** If customers abuse `&[...]` evaluation to inject complex programming scripts, Hornbill Technologies will rightfully be forced to implement strict platform-level defensive blocks, which would shut down custom expression evaluation for everyone.
> - **Workflow Designer Validation Warnings:** When pasting `&[...]` expressions into non-variable fields (such as text, date, or numeric inputs), the Hornbill BPM Designer will flag the field with a **yellow validation warning banner** (e.g. *Invalid format*). Yellow warnings **do not block publishing or activation**, and the backend V8 engine will evaluate the expression at runtime. Always test in staging before applying to production workflows.
